<template>
	<view class="app" >
		<!-- 头部 -->
		<page-header :keyword="keyword" :listType="listType" :sourcePage="sourcePage" @delKeyword="delKeyword" @changeListType="changeListType"></page-header>
		<!-- 顶部筛选 分类栏 -->
		<view class="top" :style="{ top: navigationBarHeight + statusBarHeight + 'px' }">
			...
			<view class="sort-bar row">
				...
				<view class="btn center">
					<text class="mix-icon" :class="listType === 'column' ? 'icon-hengxiangliebiao' : 'icon-shuxiangliebiao'" @click="changeListType"></text>
				</view>
				<!-- #endif -->
			</view>
			<!-- 商品分类 -->
			...
			>
				<view class="cate-wrap row">
					...
					<text
						:id="'cate-' + item.id"
						class="item"
						:class="{ active: item.id == curCateItem.id }"
		...
		<!-- 产品列表 -->
		<mescroll-body ref="mescrollRef" @init="mescrollInit" :top="headerHeight" @down="downCallback" :up="upOption" @up="loadList">
...
export default {
	...
	methods: {
		...
		async loadList(e) {
			...
			const curList = res.result.goods_list;
			if (e.num === 1) {
				//第一页清空数据重载
				this.list = [];
				this.loaded &&
		...
		async loadCateList() {
			...
			if (eventChannel.once) {
				eventChannel.once('currentCate', data => {
					this.cateList = data.child_cate;
				});
				this.$nextTick(() => {
					this.calcCateRect();
				});
		...
		calcCateRect() {
			...
			Promise.all(tasks).then(() => {
				...
				let cur = this.cateList.filter(item => item.id === this.curCateItem.id);
				if (cur.length > 0) {
		...
		changeCategory(item) {
			...
			// 	return;
			// }
			if (this.curCateItem.id === item.id) {
				return;
			}
