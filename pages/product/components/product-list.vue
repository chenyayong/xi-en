<template>
	<view class="mix-product-list" :class="{ 'is-row': listType === 'row' }">
		<view
			...
			:key="item.goods_id"
			@click="navTo('/pages/product/detail?id=' + item.goods_id)"
		>
			<view class="image-wrapper lazyload lazypic" :class="{ loaded: item.loaded }">
				<image :src="domain + item.original_img" mode="aspectFill" lazy-load="true" @load="imageOnLoad(item)"></image>
			</view>
			<view class="right column">
				<text class="title clamp">{{ item.goods_name }}</text>
				<text class="sales">销量 {{ item.sales_sum || 0 }}</text>
				<view class="price-wrap row">
					<mix-price-view :price="item.shop_price" :size="34"></mix-price-view>
		...
		>
			<view class="image-wrapper lazyload lazypic" :class="{ loaded: item.loaded }">
				<image :src="domain + item.original_img" mode="aspectFill" lazy-load="true" @load="imageOnLoad(item, 'test')"></image>
			</view>
			<text class="title clamp2">{{ item.goods_name }}</text>
			<view class="price-wrap row">
				<mix-price-view :price="item.shop_price"></mix-price-view>
				<text class="m-price">{{ item.market_price }}</text>
				<!-- <text v-if="item.market_price > item.price" class="m-price">{{ item.market_price }}</text> -->
