	...
	<uni-popup ref="popup" type="bottom">
		<view class="content column">
			...
			<view class="pro-info row">
				<image class="pic" :src="currentSku.image || domain + infoData.thumb" mode="aspectFill"></image>
			...
			<scroll-view class="c-wrap" scroll-y="true">
				<view class="sku-list fill">
					<view v-for="(item, index) in specList" :key="index">
						...
						<text class="s-tit">{{ item.item_name }}</text>
						<!-- 规格值 -->
						<view class="attrs">
							<!-- <view
							...
							<view
								class="attr round center"
								:class="{ active: aItem.checked, disabled: aItem.disabled }"
								v-for="(aItem, aIndex) in item.item_value"
								:key="aIndex"
								@click="chooseAttr(aItem, item)"
							>
								<text>{{ aItem.item }}</text>
							</view>
						</view>
					</view>
				</view>
			...
			<view class="btn-wrap row">
				<view v-if="type === 'cart' || type === 'buy'" class="confirm-btn center" @click="confirm(type)"><text>确认</text></view>
				<view v-else class="sm row">
					<view class="btn center" @click="confirm('cart')"><text>加入购物车</text></view>
...
export default {
	...
	created() {
		...
		const { specList, attrList } = this.infoData.skuData;
		const skuList = this.infoData.sku;
		...
		specList.forEach((item, index) => {
			item.item_value.forEach((aItem, aIndex) => {
				if (aIndex === 0) {
					aItem.checked = true;
					checkoutItem = aItem;
				} else {
	...
	methods: {
		...
		chooseAttr(item, parent) {
			...
			// 	}
			// });
			parent.item_value.forEach(attr => {
				attr.checked = false;
			});
			item.checked = true;
		...
		renderChecked() {
			...
			const attrs = specList.map(item => item.item_value.find(attr => attr.checked).item).join(' ');
			const ids = specList.map(item => item.item_value.find(attr => attr.checked).item_id).join('_');
			const sku = skuList.find(item => item.key == ids);
			this.currentSku = {
