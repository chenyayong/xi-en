<template>
	<view class="app">
		<mescroll-body ref="mescrollRef" @init="mescrollInit" :top="0" @down="downCallback" :up="upOption" @up="loadList">
			<!-- 列表 -->
			<uni-swipe-action ref="swipeAction">
				<uni-swipe-action-item v-for="(item, index) in list" :right-options="options" :key="index" @click="remove($event, index)">
					<view class="item row">
						<text class="mix-icon icon-xuanzhong" :class="{ active: item.checked }" @click.stop.prevent="checkRow(item, index)"></text>
						<view class="image-wrapper lazyload lazypic" :class="{ loaded: item.loaded }" @click="navTo('/pages/product/detail?id=' + item.id)">
							<image :src="domain + item.thumb" mode="aspectFill" lazy-load="true" @load="imageOnLoad(item)"></image>
						</view>
						<view class="right column">
							<text class="title clamp">{{ item.title }}</text>
							<text class="sku" v-if="item.sku && item.sku.name">{{ item.sku.name }}</text>
							<text class="price">¥{{ item.price || '' }}</text>
							<view class="number-box" @click.stop.prevent="stopPrevent">
								<uni-number-box :min="1" :max="item.stock" v-model="item.number" @change="onNumberChange($event, index)"></uni-number-box>
								<!-- <mix-number-box
									:min="1"
									:max="item.stock"
									:value="item.number"
		...
		<view v-if="loaded && (list.length > 0 || invalidList.length > 0)" class="bottom row">
			...
			<view class="del-btn center" :class="{ active: allChecked }" @click="showPopup('mixModal')"><text>清空</text></view>
...
import { getCartOrders, updateCartOrders, updateAllCartOrders, deleteCartOrders, deleteAllCartOrders } from '@/api/cart.js';
export default {
	...
	computed: {
		list: {
			get() {
				return this.$store.state.cartOrders;
			},
			set(data) {
	...
	methods: {
		...
		createOrder() {
			...
			this.navTo('/pages/order/createOrder?type=cart&ids=' + ids, {
				success: res => {
					res.eventChannel.emit('goodsData', this.list);
				}
			});
		},
		//数量修改
		onNumberChange(number, index) {
			this.$util.debounce(async () => {
				const res = await updateCartOrders(
					{
		...
		//单选
		async checkRow(item, index, checked) {
			const res = await updateCartOrders(
				{
					id: item.id,
			...
			// if (this.chenageChecked([item.id], !item.checked)) {
		...
		async checkAll() {
			...
			const res = await updateAllCartOrders(
				{
					checked: allChecked
				},
				'checked'
			...
			this.list.forEach((item, index) => {
				this.$set(item, 'checked', allChecked);
			});
		...
		remove(e, index) {
			this.$util.throttle(async () => {
				const res = await deleteCartOrders(this.list[index].id);
				if (res.status === 1) {
		...
		clear() {
			this.$util.throttle(async () => {
				const res = await deleteAllCartOrders();
				this.$util.msg(res.msg);
				if (res.status === 1) {
					this.list = [];
				}
