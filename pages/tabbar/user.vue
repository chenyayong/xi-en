	...
	<view class="app">
		<view class="top">
			...
			<view class="user-wrapper">
				<image class="avatar" :src="head_pic || '/static/icon/default-avatar.png'" @click="navTo('/pages/set/userInfo', { login: true })"></image>
				<view class="cen column" v-if="hasLogin">
					<text class="username f-m">{{ userInfo.nickname || userInfo.username }}</text>
					<text class="user-group">{{ userInfo.level_name }}</text>
				</view>
		...
		<view class="money-wrap">
			<view class="item center" hover-class="hover-gray" :hover-stay-time="50" @click="navTo('/pages/wallet/index', { login: true })">
				<text class="num">{{ userInfo.user_money | price }}</text>
				<text>余额</text>
			</view>
			<!-- <view class="item center" hover-class="hover-gray" :hover-stay-time="50" @click="navTo('/pages/coupon/userCoupon')">
				<text class="num">{{ couponCount }}</text>
				<text>优惠券</text>
			</view> -->
			<!--  hover-class="hover-gray" :hover-stay-time="50" -->
			<view class="item center" @click="navTo('/pages/points/index', { login: true })">
				<text class="num">{{ userInfo.pay_points || 0 }}</text>
				<text>积分</text>
			</view>
		</view>
		<!-- 订单 -->
		...
		<view class="option-wrap">
			...
			<mix-list-cell
				icon="icon-yishouhuo"
				iconColor="rgb(187 43 128)"
				title="我的团队"
				@onClick="navTo('/pages/teams/index', { login: true })"
			></mix-list-cell>
			<mix-list-cell
				icon="icon-shoujihaoma"
				iconColor="rgb(223 211 69)"
				title="推广海报"
				@onClick="navTo('/pages/playbill/index', { login: true })"
			></mix-list-cell>
			<mix-list-cell
			...
			<!-- <mix-list-cell
				...
				iconColor="#ee883b"
				title="意见反馈"
				@onClick="navTo('/pages/feedback/feedback', { login: true })"
			></mix-list-cell> -->
