<template>
	<view class="app">
		...
		<mescroll-body ref="mescrollRef" @init="mescrollInit" :top="headerHeight" @down="downCallback" :up="upOption" @up="loadHotList">
			<!-- #endif -->
			<!-- #ifdef MP -->
			...
			<view v-for="item in shopList" :key="item.id">
				<magic-guideline v-if="item.name === 'magic_guideline'" :componentData="item"></magic-guideline>
				<magic-swiper v-if="item.name === 'magic_swiper'" :componentData="item"></magic-swiper>
				<magic-button-group v-if="item.name === 'magic_button_group'" :componentData="item"></magic-button-group>
				<magic-picture v-if="item.name === 'magic_picture'" :componentData="item"></magic-picture>
				<magic-goods v-if="item.name === 'magic_goods'" :componentData="item"></magic-goods>
			</view>
			...
			<!-- 中部广告位 -->
			<!-- <view class="mid-ad" @click="navTo(midAdvert.link)"><image :src="midAdvert.image" mode="scaleToFill"></image></view> -->
...
import MagicGuideline from '@/components/magic/magic-guideline/index.vue';
import MagicSwiper from '@/components/magic/magic-swiper/index.vue';
import MagicPicture from '@/components/magic/magic-picture/index.vue';
import MagicButtonGroup from '@/components/magic/magic-button-group/index.vue';
import MagicGoods from '@/components/magic/magic-goods/index.vue'
...
export default {
	...
	methods: {
		...
		async getShops() {
			...
			this.shopList = JSON.parse(res.result.shop_data);
			this.pageTitle = res.result.page_title;
			this.pageIcon = res.result.page_icon;
...
.hot-wrap {
	...
	/deep/ .list-item {
		box-shadow: 4rpx 0 10rpx rgba(0, 0, 0, 0.06);
	}
}
</style>
