<template>
	<view class="app">
		<view class="main-wrap" :class="{ show: loaded }">
			...
			<view class="state-wrap column">
				<view class="row">
					<text class="state">{{ data.order_status_desc }}</text>
					<text v-if="data.timeline" class="mix-icon icon-you"></text>
				</view>
				<text v-if="data.status_tip" class="tip">{{ data.status_tip }}</text>
			</view>
			...
			<view class="addr-sec">
				<text class="mix-icon icon-dizhi"></text>
				<view class="con column">
					<text class="addr">{{ data.address }} {{ data.room }}</text>
					<text class="name">{{ data.consignee }} {{ data.mobile }}</text>
				</view>
			</view>
			<!-- 商品 -->
			<view v-if="data.products.length > 0" class="goods-sec"><product-list :list="data.products"></product-list></view>
			<!-- 价格信息 -->
			<view class="price-sec">
				<view class="cell row">
					<text class="tit fill">商品金额</text>
					<text>￥{{ data.order_amount || 0 }}</text>
				</view>
				<view class="cell row">
					<text class="tag">满</text>
					<text class="tit fill">订单满减</text>
					<text>-￥{{ data.discount || 0 }}</text>
				</view>
				<view class="cell row">
					<text class="tit fill">积分抵扣金额</text>
					<text>-￥{{ data.integral_money || 0 }}</text>
				</view>
				<view class="total row b-t">
					<text class="price">￥{{ data.total_amount || 0 }}</text>
				</view>
				<!-- <view class="cell row">
			...
			<view class="board">
				<view class="cell">
					...
					<text class="text">{{ data.order_sn || '' }}</text>
					<!-- #ifndef H5 -->
					<view class="copy-btn center round" @click="copy(data.order_sn)"><text>复制</text></view>
					<!-- <view class="copy-btn center round" @click="copy(data.order_number)"><text>复制</text></view> -->
					<!-- #endif -->
				</view>
				<!-- <view class="cell">
				...
				<view class="cell">
					<text class="tit">下单时间：</text>
					<text class="text">{{ data.add_time }}</text>
				</view>
				...
				<view v-if="data.user_note" class="cell">
					<text class="tit">订单备注：</text>
					<text class="text">{{ data.user_note }}</text>
				</view>
			</view>
			...
			<view class="page-bottom row">
				<view
					v-if="data.order_status_code === 'CANCEL' || (data.order_status === 4 && data.order_status === 5)"
					class="btn center round"
					@click="deleteOrder(data)"
				>
					<text>删除订单</text>
				</view>
				<view v-if="data.pay_status === 0 && data.order_status_code !== 'CANCEL'" class="btn center round" @click="cancelOrder(data)">
					<text>取消订单</text>
				</view>
				<view v-if="data.pay_status === 0 && data.order_status_code !== 'CANCEL'" class="btn center" @click="pay(data)"><text>立即支付</text></view>
				<!-- #ifdef MP -->
				<button type="default" open-type="contact">
					<view v-if="data.pay_status === 1" class="btn center round"><text>联系客服</text></view>
				</button>
				<!-- #endif -->

				<view
					v-if="data.pay_status === 1 && (data.order_status_code !== 'REFUND' && data.order_status_code !== 'WAITRECEIVE')"
					class="btn center round"
					@click="showRefundAction(item)"
				>
					<text>申请退款</text>
				</view>
				<!-- <view v-if="data.pay_status === 1 && data.order_status === 0" class="btn center round" @click="navTo('zizhufahuo?id=' + data.order_id)">
					<text>自助发货</text>
				</view> -->
				<!-- <view v-if="data.pay_status === 1 && data.order_status === 1" class="btn center round" @click="navToExpress(data)">
					<text>查看物流</text>
				</view> -->
				<view v-if="data.pay_status === 1 && data.order_status_code === 'WAITRECEIVE'" class="btn center" @click="confirmReceipt(data)">
					<text>确认收货</text>
				</view>
				<!-- <view v-if="data.pay_status === 1 && data.order_status === 2" class="btn center" @click="rate(data)"><text>立即评价</text></view> -->
				...
				<!-- #ifdef MP -->
				<!-- <button type="default" open-type="contact">
					<view v-if="data.status === 1" class="btn center round" @click="cancelOrder(data)"><text>联系客服</text></view>
				</button> -->
				<!-- #endif -->
				<!-- #ifndef H5 -->
				<!-- <view v-if="data.status === 1" class="btn center round" @click="showRefundAction(data)"><text>申请退款</text></view> -->
				<!-- #endif -->
				<!-- <view v-if="data.status===1" class="btn center round" @click="navTo('zizhufahuo?id='+data._id)">
					<text>自助发货</text>
				</view> -->
				<!-- <view v-if="data.status === 2" class="btn center round" @click="navToExpress(data)"><text>查看物流</text></view>
		...
		<uni-popup v-if="data.timeline" ref="timelinePopup">
			<view class="timeline-content">
				<scroll-view class="timeline-scroll" scroll-y="true"><mix-timeline :list="data.timeline"></mix-timeline></scroll-view>
...
export default {
	...
	methods: {
		async loadData() {
			...
			this.data = res.result.order_info;
			this.data.products = res.result.order_info.goods_list.map(item => {
				return {
