<template>
	<view class="app">
		<view class="main-wrap" :class="{ show: loaded }">
			<view class="item column" v-for="(item, index) in list" :key="item.address_id" @click="chooseAddress(item)">
				<view class="row">
				...
				<text class="content">{{ address(item) }}</text>
				<view class="bot row b-t" @click.stop.prevent="stopPrevent">
					...
					<!-- <view v-if="item.is_default !== 1" class="btn center" @click="setDefalutAddr(item)">
...
<script>
import { getAddress, deleAddress } from '@/api/address.js';
export default {
	data() {
		return {
			list: []
	...
	methods: {
		...
		async loadData() {
			...
			this.list = res.result.address_lists;
			this.loaded = true;
		...
		async deleteAddr() {
			...
			// 	}
			// );
			const res = await deleAddress({
				id: this.list[this.curIndex].address_id
			});
