	...
	<view class="app">
		<uni-list v-if="list && list.length">
			<uni-list-chat
				...
				:time="item.money"
				:badge-text="item.status_desc"
			></uni-list-chat>
		</uni-list>
		<mix-empty v-else></mix-empty>
		<!-- <mix-empty v-else type="cart"></mix-empty> -->
		<!-- <mescroll-body ref="mescrollRef" @init="mescrollInit" :top="0" @down="downCallback" :up="upOption" @up="loadList">
			<view class="item b-b" :class="{ income: item.money > 0 }" v-for="(item, index) in list" :key="index">
				<text
					class="mix-icon"
					:class="item.type === 'recharge' ? 'icon-chongzhi' : item.type === 'withdraw' ? 'icon-tixian1' : 'icon-zhangdanmingxi01'"
				></text>
...
import { getWithrawalsList } from '@/api/user.js';
export default {
	...
	methods: {
		...
		async loadData(type) {
			this.isLoading = true;
			const res = await getWithrawalsList(this.params);
			if (res.status === 1) {
				if (type === 'add') {
...
.app /deep/ .uni-list-chat__header-warp {
	display: none;
}
.item {
	display: flex;
