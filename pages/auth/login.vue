		...
		<view class="wrapper">
			...
			<!-- <view class="input-content">
				<view class="input-item">
					...
					<view class="row">
						<input v-model="username" type="number" maxlength="11" placeholder="请输入手机号码" placeholder-style="color: #909399" />
					</view>
				</view>
				<view class="input-item">
			...
			<view class="other-wrapper">
				<view class="line center"><text class="tit">快捷登录</text></view>
				<view class="list row">
					
					<!-- #ifdef H5 -->
					<view class="item column center" @click="h5WxGetUserInfo"><image class="icon" src="/static/icon/login-wx.png"></image></view>
					<!-- #endif -->
					
					<!-- #ifdef MP-WEIXIN -->
					<view class="item column center" @click="mpWxGetUserInfo"><image class="icon" src="/static/icon/login-wx.png"></image></view>
					<!-- #endif -->
...
import loginH5 from './mixin/login-h5.js';
import { userLogin } from '@/api/user.js';
export default {
	mixins: [loginMpWx, loginAppWx, loginApple, loginH5],
	data() {
		return {
	...
	methods: {
		...
		async login() {
			...
			const data = { mobile: username, mobile_code: code }
			data.platform = 1
			data.invite = this.$store.state.invite
			// #ifdef H5
			data.platform = 2
			// #endif
			...
			if (res.status === 1) {
				this.loginSuccessCallBack({ token: res.result });
			} else {
